...
int main()
{
    ...
    if (ListInsert1(&L, e2, e1, MyCompare) == OK) {
        ...
        printf("在%s前插入元素=%s成功\n", e2, e1);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
        printf("在\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"成功\n", e2.num, e2.name, e2.sex, e2.score, e2.addr, e1.num, e1.name, e1.sex, e1.score, e1.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
        printf("在\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"成功\n", e2->num, e2->name, e2->sex, e2->score, e2->addr, e1->num, e1->name, e1->sex, e1->score, e1->addr);
#else	//缺省当做int处理
        printf("在%d前插入元素=%d成功\n", e2, e1);
        ...
        printf("在%s前插入元素=%s失败\n", e2, e1);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
        printf("在\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"失败\n", e2.num, e2.name, e2.sex, e2.score, e2.addr, e1.num, e1.name, e1.sex, e1.score, e1.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
        printf("在\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"失败\n", e2->num, e2->name, e2->sex, e2->score, e2->addr, e1->num, e1->name, e1->sex, e1->score, e1->addr);
#else	//缺省当做int处理
        printf("在%d前插入元素=%d失败\n", e2, e1);
