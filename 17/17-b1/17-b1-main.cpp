/* 2152131 吴洪蕊 计科 */
/* 2250898 陆思宇 计科 2351114 朱俊泽 大数据 2253767 李昊林 信安 */
...
int main()
{
	...
	if (1) {
		...
		const char arr[] = "teststr2";

		s1.assign(TStringAdv("ABC"));
		cout << "s1 应该是 ABC，实际输出：" << s1 << endl;

		s1.assign(cstr);
		cout << "s1 应该是 Hello World，实际输出：" << s1 << endl;

		s1.assign("");
		cout << "s1 应该是 <EMPTY>，实际输出：" << s1 << endl;

		s1.assign(arr);
		cout << "s1 应该是 teststr2，实际输出：" << s1 << endl;

		s1.assign("First").assign("Second");
		cout << "s1 应该是 Second，实际输出：" << s1 << endl;
	...
	if (1) {
		...
		s1.append(s2);
		cout << "s1 应该是 HelloWorld，实际输出：" << s1 << endl;
		...
		// append(char 数组)
		s1.append(arr);
		cout << "s1 应该是 HelloWorld!123，实际输出：" << s1 << endl;
		...
		// 连续 append 检验
		s1 = "Get";
		s1.append(" ").append("in").append(" ").append("there, ").append("Lewis!");
		cout << "s1 应该是 Get in there, Lewis!，实际输出：" << setw(20) << s1 << endl;
	...
	if (1) {
		...
		TStringAdv s1("12345678"), s2("abc");
		const char* cstr = "789";
		const char arr[] = "#";

		// insert(const TStringAdv &ts2, int pos)
		s1.insert(s2, 4);
		cout << "s1应为123abc45678，实际输出：" << s1 << endl;

		// insert(const char *s, int pos)
		s1.insert(cstr, 2);
		cout << "s1应为178923abc45678，实际输出：" << s1 << endl;

		// insert(字符数组,中间位置)
		s1.insert(arr, 7);
		cout << "s1应为178923#abc45678，实际输出：" << s1 << endl;
	...
	if (1) {
		...
		// 测试 erase(const TStringAdv &ts2)
		s1.erase(str2);
		cout << "s1应该为apple ，                      实际：" << s1 << endl;
		...
		s2.erase(str1);
		cout << "s2应该为pie ，                    实际：" << s2 << endl;

		// 测试 erase(const char &c)（删除‘p’）
		s2.erase(c);
		cout << "s2应该为ie ，                      实际：" << s2 << endl;

		// 连续删除
		s4.erase(c1).erase(c1);
		cout << "s4应该为Apie ，                        实际：" << s4 << endl;
	...
	if (1) {
		...
		cout << "sub应为Tongj，         实际输出：" << sub << endl;

		// 超出范围的len截断到字符串末尾
		sub = s1.substr(7, 50);
		cout << "sub应为University，    实际输出：" << sub << endl;
	...
	if (1) {
		cout << "/*************************     at Test         *****************************/" << endl;
		TStringAdv s1 = "apple";
		TStringAdv s2[4] = { "orange", "banana", "grape", "pear" };
		int i, j;
		char ch = 'P';
	...
	if (1) {
		...
		cout << "求长度测试(TStringAdvLen()函数) 这个是不是直接copy上面的就可以？" << endl;
